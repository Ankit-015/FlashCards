...
export default function App() {
  ...
  return (
    <>
      {/*Fixed the prop driliing here by sending component as children i.e. component composition*/}

      <Navbar>
        <Search query={query} setQuery={setQuery} />
        <NumResult movies={movies ? movies : 0} />
      </Navbar>
...
function WatchedMoviesList({ watched, onDeleteWathced }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovies
...
function WatchedMovies({ movie, onDeleteWathced }) {
  return (
    <li key={movie.imdbID}>
      <img src={movie.poster} alt={`${movie.title} poster`} />
      <h3>{movie.title}</h3>
